...
class TextGenerator:
    def __init__(self, 
                 checkpoint='model/base_model_215M/pretrain_1024_18_8192.pth',  # 模型检查点路径
                 tokenizer_model_path='model/tokenizer/',  # 分词器模型路径
                 seed=42,  # 随机种子，确保可重复性
                 device=None,  # 设备，优先使用 CUDA，如果没有可用的 CUDA，则使用 CPU
                 dtype="bfloat16"):  # 数据类型，默认为 float32，可以选择 float16 或 bfloat16
        """
        [NOTE] __init__ 学不学，还是就看一遍，学的话怎么学，始终是一个问题
        ...
        # [NOTE] 混合精度是什么意思？不同的张量的数据类型不同，由上下文管理器自动决定，来平衡消耗和性能吗？
        ...
        self.model = Transformer(ModelConfig(vocab_size=8192, dim=1024, n_layers=18))  # 实例化 Transformer 模型
    ...
    def sft_sample(self, 
        ...
        :return: 生成的文本样本列表
        """
        start = self.chat_template(start)  # 经过了sft，就要应用对话模板了
        # 将起始文本编码为 token id 序列
        # [NOTE] 这里怎么又有 data 了？很迷惑。标准的结构如何？
        start_ids = self.tokenizer(start)['input_ids'] 
        # print('start_ids:', start_ids)
        # [NOTE] 用 (xxx[None, ...]) 包围有什么作用？
        # 主要作用是增加一个维度，符合 generate 对输入的 [batch_size, seqlen] 的要求
        # start_ids 是 Token 的索引（如 1, 99, 5602），它们必须是 整数 (Long/Int64)，如果浮点数会报错。所以 dtype=torch.long 是固定的
        ...
        with torch.no_grad():  # 禁用梯度计算，提升效率
            with self.ctx:  # 进入自动混合精度的上下文（如果是 GPU 并使用 float16 时）
                # [NOTE] num_samples 和 max_new_tokens 的区别？num_samples 是多生成几次让回答长一些吗
                # LLM 能否被视为一个 N 很大的 N-grams 模型？区别和联系？
                for i in range(num_samples):  # 循环生成指定数量的样本
                    y = self.model.generate(x, self.tokenizer.eos_token_id, max_new_tokens, temperature, top_k)  # 生成文本
                    # [NOTE] 这里假定了 batch_size = 1
    ...
    def pretrain_sample(self, 
        ...
        """
        根据给定的起始文本生成样本。
        和 sft_sample 基本一样，还更简单一些。不用 apply_chat_template
...
if __name__ == "__main__":
    ...
    # [NOTE] <|im_start|> 加与不加，和前后逻辑有关。确认一下
    pretrain_prompt_datas = [
        '<|im_start|>北京大学是',
        '<|im_start|>哈基米南北绿豆',
    ]

    generator = TextGenerator(checkpoint='model/base_model_215M/pretrain_1024_18_8192.pth')  # 初始化生成器
    ...
    generator = TextGenerator(checkpoint='model/sft_model_215M/sft_dim1024_layers18_vocab_size8192.pth')  # 初始化生成器
